...
export default function ProductFlip() {
  ...
  return (
    <section className="bg-white py-12 relative select-none">
      <div className="px-4 md:px-16 w-full flex flex-col items-center justify-center">
        <span className="uppercase font-medium text-center text-xs tracking-[2px] opacity-60 mb-4">
          The Latest Trends In Athletic Footwear
        </span>
        <h2 className="font-semibold text-4xl md:text-5xl mb-8 text-black text-center">
          Sneakers & Kicks
        </h2>

        {/* Tab Buttons */}
        <div className="flex gap-4 sm:gap-8 md:gap-12 relative pb-2 justify-center border-b border-gray-200 w-full max-w-md px-4 mx-auto mb-10">
      ...
      <div className="container max-w-7xl px-4 md:px-12 mx-auto relative group-container">
        <div
        ...
        >
          <Swiper
            ...
            className="w-full mb-12 container-swiper"
          >
            {filteredProducts.map((product) => {
              ...
              return (
                <SwiperSlide key={product.id}>
                  <div className="group/card relative overflow-hidden bg-white p-2 flex flex-col justify-between h-full min-h-115 transition-all duration-300">
                    {/* Images & Badges Section */}
                    <div className="relative h-80 w-full rounded overflow-hidden bg-gray-50 flex items-center justify-center cursor-pointer">
                      {/* Badges */}
                      {isSoldOut && (
                        <span className="absolute top-3 left-3 bg-white text-black font-semibold text-[10px] tracking-wider uppercase px-2.5 py-1 z-30 shadow-sm rounded-sm">
                          SOLD OUT
                        </span>
                      )}
                      {/* Images Switch on Hover */}
                      <img
                        alt={product.name}
                        className="object-cover w-full h-full absolute inset-0 transition-all duration-500 ease-in-out opacity-100 group-hover/card:opacity-0"
                        src={product.img1}
                      />
                      <img
                        alt={`${product.name} alternate`}
                        className="object-cover w-full h-full absolute inset-0 transition-all duration-500 ease-in-out opacity-0 group-hover/card:opacity-100 scale-100 group-hover/card:scale-105"
                        src={product.img2 || product.img1}
                      />
                    </div>

                    {/* Details Section */}
                    <div className="relative z-10 pt-4 pb-2 bg-white transition-transform duration-300 ease-out group-hover/card:-translate-y-20">
                      <strong className="block font-medium text-[#333333] text-[16px] mb-0.5">
                        ${product.price.toFixed(2)}
                      </strong>

                      <h3 className="text-[20px] md:text-[20px] font-medium text-black truncate">
                        {product.name}
                      </h3>

                      <p className="text-[16px] text-[#33333399] mt-0.5">
                        {product.brand}
                      </p>
                    </div>

                    {/* Add To Cart Hover Button */}
                    <div className="absolute inset-x-0 bottom-0 py-4 bg-white transition-transform duration-300 ease-out transform translate-y-full group-hover/card:translate-y-0 z-20">
                      <div className="flex items-start justify-between gap-3.5 px-2">
                        <div className="flex text-black font-medium text-[14px] rounded transition-colors gap-2 items-center cursor-pointer">
                          <svg
                            className="svg-cart"
